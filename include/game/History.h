...
public:
	...
	std::string getLastMove(const int board_size);
	std::string getKthMove(const int k, const int board_size);
	std::vector<Move> getMoveHistory();
	std::vector<std::string> getFullMove(const int board_size);
	int getMoveListSize();
